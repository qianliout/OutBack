...
LSTM通过其独特的**加性状态更新机制**来缓解梯度消失问题，这与基础RNN中的乘性更新形成对比。1.  **近乎线性的信息流**：回顾记忆单元的更新公式：    $\mathbf{C}_t = \mathbf{F}_t \odot \mathbf{C}_{t-1} + \mathbf{I}_t \odot \tilde{\mathbf{C}}_t$    在反向传播时，梯度沿时间步从 $C_t$ 传递到 $C_{t-1}$。其关键部分的导数为：    $\frac{\partial C_t}{\partial C_{t-1}} = \mathbf{F}_t$    与基础RNN中梯度必须乘以一个固定的权重矩阵 $W_{hh}$ 不同，LSTM中的梯度乘以**遗忘门 $F_t$ 的输出**。2.  **门控的调节作用**：遗忘门 $F_t$ 的值在 `[0, 1]` 区间内，并且是根据当前输入和前一时刻状态动态计算的。这意味着网络可以**学会控制梯度的流动**：    *   当需要保留长期记忆时，网络可以学习将 $F_t$ 的值设置为接近1。此时，梯度可以几乎无衰减地向前传播，从而避免了梯度消失。    *   当过去的记忆不再重要时，网络可以将 $F_t$ 的值设置为接近0，从而“关闭”梯度流，避免不相关信息干扰。这种结构创建了一条“信息高速公路”，使得梯度能够更有效地在长序列中传播，而不会像在基础RNN中那样快速地指数级衰减。
