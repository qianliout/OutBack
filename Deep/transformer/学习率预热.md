...
余弦退火（Cosine Annealing）是一种学习率调度策略，通过余弦函数动态调整学习率，帮助模型在训练中更高效地收敛。其核心思想是**模拟余弦函数的周期性变化**，让学习率从初始值平滑下降到最小值，再重新升温，循环往复。以下是详细解析：

---

## **1. 基本公式**
余弦退火的学习率 $\eta_t$ 在每轮迭代中按以下公式更新：
$$
\eta_t = \eta_{\text{min}} + \frac{1}{2} (\eta_{\text{max}} - \eta_{\text{min}}) \left(1 + \cos\left(\frac{T_{\text{cur}}}{T_{\text{max}}} \pi\right)\right)
$$
- **$\eta_{\text{max}}$**：学习率上限（初始值）。
- **$\eta_{\text{min}}$**：学习率下限（通常设为 $\eta_{\text{max}} \times 10^{-3}$ 或更小）。
- **$T_{\text{cur}}$**：当前迭代步数（或 epoch 数）。
- **$T_{\text{max}}$**：一个周期（cycle）的总迭代步数。

---

## **2. 工作原理**
- **余弦下降**：学习率从 $\eta_{\text{max}}$ 平滑衰减到 $\eta_{\text{min}}$，遵循余弦函数的半个周期（$0 \rightarrow \pi$）。
- **周期性重启**：当 $T_{\text{cur}} = T_{\text{max}}$ 时，重置 $T_{\text{cur}}=0$，学习率跳回 $\eta_{\text{max}}$，开始新一轮下降（如下图）。


---

## **3. 为什么有效？**
1. **逃离局部最优**：周期性重启的“升温”机制帮助模型跳出局部极小值。
2. **精细调优**：学习率下降阶段允许模型在损失平面平缓区域精细调整参数。
3. **平衡探索与利用**：高温（高学习率）阶段探索新区域，低温阶段收敛到局部最优。

---

## **4. 变体：带热重启的余弦退火（SGDR）**
2017年提出的 **SGDR（Stochastic Gradient Descent with Warm Restarts）** 是余弦退火的改进版：
- **动态调整周期长度**：每次重启后，$T_{\text{max}}$ 按倍数增长（如 $T_{\text{max}} \times 2$）。
- **示例公式**：
  $$
  T_{\text{max}}^{(i)} = T_0 \cdot 2^i
  $$
  其中 $i$ 是重启次数，$T_0$ 是初始周期长度。
...
| 参数          | 作用                                                                 | 典型值                     |
|---------------|----------------------------------------------------------------------|----------------------------|
| $\eta_{\text{max}}$ | 初始学习率（根据任务设定）                                           | 0.1（图像）、1e-3（NLP）   |
| $\eta_{\text{min}}$ | 最小学习率                                                           | $\eta_{\text{max}} \times 10^{-3}$ |
| $T_{\text{max}}$   | 一个周期的长度（epoch 数或迭代步数）                                 | 10~50 epoch                |
| 重启次数       | 控制总周期数（SGDR 中可动态调整）                                    | 3~5 次                     |
...
scheduler = CosineAnnealingLR(optimizer, T_max=50, eta_min=1e-4)  # 每50 epoch重启

# SGDR（带热重启）
scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=2)  # 初始周期10 epoch，每次翻倍
...
## **7. 适用场景**
- **非凸优化问题**：如深度学习中的复杂损失曲面。
- **小批量训练**：配合 SGD 或 Adam 使用。
- **需要精细调参的任务**：图像分类、语义分割等。

通过灵活调整周期长度和重启策略，余弦退火能显著提升模型性能（如 ResNet 在 ImageNet 上的表现）。如果需要进一步讨论其他调度策略（如 OneCycleLR），可以继续提问！