...
- TERRY.ROD: 1. stress和sysbench两个工具在压测过程中的对比发现：stress基于多进程的，会fo...
- echo: 老师你好，有个难题希望能指导一下排查我有个系统，跑的是32核48G的云主机，load经常超过CPU核...

### 技术讨论

- lucian: 过多上下文切换会缩短进程运行时间
vmstat 1 1：分析内存使用情况、cpu上下文切换和中断的次...
- 发条橙子 。: 案例分析 ：登录到服务器，现在系统负载怎么样 。 高的话有三种情况，首先是cpu使用率 ，其次是io...

### 实践经验

- 行者: 结合前两节，首先通过uptime查看系统负载，然后使用mpstat结合pidstat来初步判断到底是...
- Haku: Ubuntu16.04LTS下:# 以 10 个线程运行 5 分钟的基准测试，模拟多线程切换的问题$...

### 问题求助

- 郝中阳: 我用的centos，yum装的sysbench。执行后很快完事了的可以设置下max-requests...
- CYH: 老师请教一下：我的是centos7系统，执行sysbetch后，watch -d cat /proc...
- 姜小鱼: 老师 为什么我执行sysbench之后很快就结束了？sysbench --num-threads=1...
