...
这里，我推荐从 /proc//smaps 入手。前面内容里，我并没有直接讲过/proc/smaps 文件中各个指标含义，所以，需要你自己动手查 proc 文件系统的文档，解读并回答这个问题。
...
- 我来也: [D16打卡]只有一块磁盘,就没轻易的试第二个案例.------------------------...
- 虎虎❤️: 通过读csapp，又复习了下虚拟内存。其概念为 “虚拟内存组织为一个由存放在磁盘上的N个连续的字节大...

### 问题求助

- 倪朋飞: 关于磁盘和文件的区别，本来以为大家都懂了，所以没有细讲。磁盘是一个块设备，可以划分为不同的分区；在分...
- Geek_5258f8: 理论上，一个文件读首先到Block Buffer, 然后到Page Cache。有了文件系统才有了P...
- JJ: 还是有点困惑，感觉读写磁盘上的数据不就是读写磁盘上的文件里的数据嘛，难道读磁盘上的数据可以不经过文件...
