...
- 我来也: [D17打卡]想不到Buffer 和 Cache还有专门的工具分析, 长见识了!暂时只能在自己的机器...
- 2xshu: 老师你好，第一个案列我有不太明白的地方。希望能得到老师的指教。既然执行了 echo 3 > /pro...

### 实践经验

- 往事随风，顺其自然: 要是centos验证一下就好了，不同系统很多问题不一样，操作上遇到问题很奇怪...

### 问题求助

- Johnson: dd命令也支持直接IO的 有选项oflag和iflag 所以dd也可以用来绕过cache buff做...
- jssfy: 请问第一次读50%的命中率如何解释呢？...
- noma: pcstat 不使用代理安装：if [ $(uname -m) == "x86_64" ] ; th...
