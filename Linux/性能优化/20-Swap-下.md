...
- Geek_2b6807: 希望老师在用工具的时候能够使用对内核版本要求不高的，毕竟生产环境用较新内核的还是比较少...
- ninuxer: 打卡day21按我的理解，swapness只是几率，并不意味着一定，所以还是会发生匿名页交换，只是几...
- 流转千回: 打卡，老师这么晚还在更新专栏，致敬！...

### 技术讨论

- Scott: 答案不是上一讲有提到吗，就算设置为0，如果空闲内存+文件页 < page_low，还是会发生swap...
- 仲鬼: 老师好，我研究man sar后还是没理解kbcommit，这个估计值具体指什么呢？就算单纯以不导致O...

### 问题求助

- Free_fish: 用smem --sort swap命令可以直接将进程按照swap使用量排序显示...
- 尘封: $ swapoff -a && swapon -a，线上使用这个命令释放swap有什么风险吗？...
- berryfl: /proc/*/status会匹配到self之类的特殊目录，把匹配的部分写复杂点可以限制到仅数字，例...
